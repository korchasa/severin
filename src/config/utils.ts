...
/**
 * Type-safe environment variable getter for optional values with automatic type inference
 * @param name Environment variable name
 * @param defaultValue Default value (optional). If not provided, returns undefined for missing variables
 * @returns Environment variable value converted to the type of defaultValue, or defaultValue/undefined if variable not set
 * @throws {Error} if conversion fails for existing variable
 */
export function envOptional<T extends string | number | boolean>(
  name: string,
  defaultValue?: T,
): T | undefined {
  const value = Deno.env.get(name);
 ...
 * @deprecated Use envOptional instead for better type safety
 */
export function envNumberOptional(name: string, defaultValue?: number): number | undefined {
  return envOptional(name, defaultValue);
}
